...
if (sys.nframe() == 0L) {
  ...
  for (beta in c(0.34, 0)) {
    ...
    g <- g[g$tick > 0, ]
    collected[[length(collected) + 1]] <- do.call(rbind, lapply(names(stages),
      function(k) data.frame(
        run = if (beta > 0) "beta = 0.34" else "beta = 0 (supply line ignored)",
        position = factor(stages[[k]], levels = unname(stages)),
        week = g$tick, order = g[[k]])))
    cat(sprintf("\nbeta = %.2f  (%s)\n", beta,
  ...
  # --- figure -------------------------------------------------------------
  # Time series: orders placed at each of the four positions. The bullwhip is
  # the growing amplitude as the signal travels upstream.
  library(ggplot2)
  ...
  fig <- ggplot(d, aes(week, order, colour = position)) +
    geom_line(linewidth = 0.6) +
    facet_wrap(~run, ncol = 1) +
    theme_minimal() +
    labs(title = "Beer game: one step in demand, amplified up the chain",
         subtitle = "customer demand went from 4 to 8 cases in week 5, once",
         x = "week", y = "cases ordered", colour = NULL)
  print(fig)
  ggsave(fig_file("46-beer-distribution-game.png"), fig, width = 6, height = 4,
         dpi = 120)
}
