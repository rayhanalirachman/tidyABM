...
if (sys.nframe() == 0L) {
  ...
  for (nm in names(runs)) {
    ...
    collected[[nm]] <- data.frame(regime = nm, share = x)
  ...
  # --- figure -------------------------------------------------------------
  # Final-state distribution: where the colony spends its time. The whole
  # claim is that this is U-shaped when c < 1 and single-peaked when c > 1.
  library(ggplot2)
  ...
  fig <- ggplot(d, aes(share)) +
    geom_histogram(binwidth = 0.05, boundary = 0, fill = "grey30") +
    facet_wrap(~regime, ncol = 1, scales = "free_y") +
    theme_minimal() +
    labs(title = "Kirman's ants: herding is a U-shaped stationary distribution",
         subtitle = "60 ants, two identical sources, 3000 ticks after burn-in",
         x = "share of the colony at source A", y = "ticks")
  print(fig)
  ggsave(fig_file("42-kirmans-ants.png"), fig, width = 6, height = 4, dpi = 120)
}
