...
if (sys.nframe() == 0L) {
  ...
  # --- figure -------------------------------------------------------------
  # Parameter sweep on log-log axes: both scaling laws against the N^1.5 line
  # Baronchelli et al. report.
  library(ggplot2)
  ...
  d <- rbind(
    data.frame(n = f[, "n"], law = "N_w max",        value = f[, "nw"]),
    data.frame(n = f[, "n"], law = "t_conv",         value = f[, "tconv"]),
    data.frame(n = f[, "n"], law = "N^1.5 (slope 1.5)",
               value = f[, "n"]^1.5))

  fig <- ggplot(d, aes(n, value, colour = law)) +
    geom_line(linewidth = 0.7) + geom_point() +
    scale_x_log10() + scale_y_log10() +
    theme_minimal() +
    labs(title = "Naming game: both scaling laws come out at N^1.5",
         subtitle = "3 replicates per size, log-log axes",
         x = "agents (N)", y = "words held / interactions to converge",
         colour = NULL)
  print(fig)
  ggsave(fig_file("40-naming-game.png"), fig, width = 6, height = 4, dpi = 120)
}
