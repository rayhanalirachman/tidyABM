# The non-spatial corpus: 56 models
...
the numbers the run produced and a figure of the result. This is one half of
[the corpus](../README.md); the other is the ten lattice models in
[`../spatial/`](../spatial/README.md). Every model here runs. Every one with a
behavioural claim is pinned by a test in `../../tests/testthat/`.
...
Each script also writes the figure its page embeds, into `figures/`. The
`testthat` cases run the same models at reduced scale so the suite stays quick.
The scripts are what reproduce the published tables.

```
Rscript models/non-spatial/scripts/43-zero-intelligence-traders.R
```

The scripts need `tidyABM` and `ggplot2` installed.
