...
#'
#' Two [abm_neighbours()] passes over the same network draw their random
#' numbers independently, which is fine when each pass is a separate event and
#' wrong when it is the *same* event seen from two sides. "How many of my
#' neighbours punished me" and "how much punishing did I do" have to add up to
#' the same thing, agent by agent, and they cannot if each aggregate rolls its
#' own dice.
#'
#' `abm_draw()` fixes that by putting the draw on the **edge** rather than in
#' the aggregate. Each rule is evaluated once per edge, and the value it
#' produces is then readable inside every later [abm_neighbours()] rule in the
#' same tick, from either endpoint, as a column of that name. Because both
#' endpoints read the same number, two aggregates over it describe the same
#' events.
#'
#' The right-hand side is evaluated over the **edge table**, one row per edge,
#' with `from` and `to`, so `n()` is the number of edges and `runif(n())` is "a
#' uniform per edge". Globals are in scope. The values live until the next
#' `abm_draw()` of that name replaces them, so a draw made once per tick lasts
#' the tick and one made in `abm_setup()`'s first step lasts the run.
#'
