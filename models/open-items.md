...
collecting more. Seven of the nine went; the two that remain are the two that
were never requests for expressiveness.
...
- **~~No neighbourhood in attribute space.~~** `abm_neighbours(..., within =)`
  ...
  and produces the same trajectory, agent for agent, as the hand-rolled O(n²)
...
- **~~`abm_birth()` gives one offspring per parent.~~** `times =` is an
  ...
  evaluated once, in the parent's row — reproduction is paid for per parent, not
...
One fix came off with them. A value looked up out of a keyed global —
...
- **`resolve = "negotiate"` is narrow.** It implements one bargaining protocol:
  ...
  round it — an order book written with `abm_sequential()` and `abm_tell()` —
  but that is twenty rules, not an argument. The reason this one stays open is
