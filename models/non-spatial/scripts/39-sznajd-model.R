...
if (sys.nframe() == 0L) {
  ...
  for (p in ps) {
    ...
    sweep[[length(sweep) + 1]] <- data.frame(
      p_up = p, exit = mean(out[done, "up"] == 1))
  }
  ...
  # --- figure -------------------------------------------------------------
  # Parameter sweep: the exit probability against the initial density, with
  # the voter model's identity line for comparison.
  library(ggplot2)
  ...
  fig <- ggplot(sweep, aes(p_up, exit)) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed",
                colour = "grey50") +
    geom_line(linewidth = 0.7) + geom_point() +
    xlim(0, 1) + ylim(0, 1) +
    theme_minimal() +
    labs(title = "Sznajd: outflow dynamics beat the voter model's identity",
         subtitle = "ring of 16, 30 replicates per point; dashed line is the voter model",
         x = "initial density of +1", y = "P(consensus is +1)")
  print(fig)
  ggsave(fig_file("39-sznajd-model.png"), fig, width = 6, height = 4, dpi = 120)
}
