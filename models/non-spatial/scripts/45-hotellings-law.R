...
if (sys.nframe() == 0L) {
  ...
  # --- figure -------------------------------------------------------------
  # Parameter sweep: how far buyers actually walk against how far they would
  # if the shops were placed to suit them. The gap is Hotelling's point.
  library(ggplot2)
  ...
  d <- rbind(
    data.frame(shops = sweep$shops, placement = "where shops settle",
               walk = sweep$walk),
    data.frame(shops = sweep$shops, placement = "social optimum",
               walk = sweep$best))

  fig <- ggplot(d, aes(factor(shops), walk, fill = placement)) +
    geom_col(position = "dodge") +
    theme_minimal() +
    labs(title = "Hotelling's Law: minimum differentiation costs the buyers",
         subtitle = "100-unit beach, 400 buyers, 300 ticks, 5 seeds",
         x = "shops", y = "mean distance walked", fill = NULL)
  print(fig)
  ggsave(fig_file("45-hotellings-law.png"), fig, width = 6, height = 4,
         dpi = 120)
}
