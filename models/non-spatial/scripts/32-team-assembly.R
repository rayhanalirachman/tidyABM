...
sweep <- do.call(rbind, lapply(
  list(c(.2, .5), c(.5, .5), c(.8, .5), c(.95, .5), c(.5, .95)), function(par) {
    r <- team_assembly(par[1], par[2])
    g <- giant_share(r)
    cat(sprintf("p = %.2f  q = %.2f   giant component = %.2f of the collaborators\n",
                par[1], par[2], g))
    data.frame(p = par[1], q = par[2], giant = g)
  }))

# --- figure ---------------------------------------------------------------
# Parameter sweep: the giant component against the incumbency rate p, which is
# the transition Guimera et al. report.
library(ggplot2)
...
fig <- ggplot(d, aes(p, giant)) +
  geom_line(linewidth = 0.7) + geom_point() +
  ylim(0, 1) +
  theme_minimal() +
  labs(title = "Team assembly: incumbency builds the collaboration network",
       subtitle = "giant component among collaborators, q = 0.5, 300 teams",
       x = "probability a new member is an incumbent (p)",
       y = "giant component, share of collaborators")
print(fig)
ggsave(fig_file("32-team-assembly.png"), fig, width = 6, height = 4, dpi = 120)
