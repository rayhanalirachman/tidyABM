...
- `abm_network(type =, degree =, edges =)` — a persistent edge list.

| `type` | what it builds |
|---|---|
| `"random"` | a `degree`-regular random graph — every agent has *exactly* `degree` neighbours |
| `"poisson"` | Erdős–Rényi `G(n, p)` with mean degree `degree` — a spread of degrees |
| `"scale_free"` | Barabási–Albert, `degree` edges per new agent — a heavy tail |
| `"ring"` | the one-dimensional lattice, `degree / 2` neighbours each side |
| `"complete"` | every pair — the well-mixed population, written as a graph |
...
- `"random"` — shuffle into groups of `size` (default 2)
- `"opposite_group"` — pair across a two-valued split named by `by`; supports
...
- `"one_of"` — a partner drawn uniformly from the candidates (NetLogo's
  `one-of other turtles`)
- `"nearest"` — the closest candidate in the space named by `by`, or the one
  that minimises the expression named by `cost`
- `"network"` — a neighbour from the model's network; `from = "random_edge"` or
  `from = "parent"` inside `abm_birth(attach_via =)`
...
— two wolves cannot eat the same sheep — and a directional one does not. Where
matching stands in for a rate rather than a rule, the mode *is* the model:
`"opposite_group"` makes `min(A, B)` encounters and a directional draw makes as
many as there are choosers, and in a predator–prey model those are
...
including `.id` and `.group`. `NA` means the candidate is not acceptable to that
...
For pairs, `partner_<col>` for every column of the partner — including list
columns. For groups, grouped evaluation, so `sum(x)` inside a rule means "across
...
sees the whole population — `n()` is everybody — because a colony-level balance
is about the colony.
...
rules cascade within the agent** — the second rule sees what the first one just
wrote. That is what "one agent at a time" implies, and it is what lets an agent
...
an agent out. The difference is not cosmetic — the same bank-run model under the
two orderings gives different answers, because a fixed queue is information an
agent has about itself (model 52).
...
once per edge — `n()` is the number of edges — and every later
...
the value lands in the **recipient's** column of that name. `to = "neighbours"`
broadcasts to everyone the sender is joined to; `to = <expression>` names one
recipient by `.id`, so `to = .partner` writes to your match partner and
...
to all of them — an audience that is neither its partner nor its network.

`.resolve` says what happens when two senders address the same recipient:
`"last"`, `"first"`, `"sum"`, `"mean"`, `"max"`, `"min"`, `"collect"` — which
hands the recipient the list of everything it was told and lets its own rule
decide — or `"error"` to stop. An agent nobody wrote to keeps what it had.
...
way an `abm_global()` right-hand side is — over the whole population, with the
...
whatever it says. A fixed population can ignore this; a growing one cannot,
