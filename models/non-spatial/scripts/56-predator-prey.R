...
if (sys.nframe() == 0L) {
  ...
  # --- figure ---------------------------------------------------------------
  # Time series: both populations against tick. The quarter-period lag is the
  # whole result, and it is visible as the offset between the two curves.
  library(ggplot2)
  ...
  d <- rbind(data.frame(tick = g$tick, species = "sheep",
                        n = g$n_sheep),
             data.frame(tick = g$tick, species = "wolves",
                        n = g$n_wolves))
  d <- d[d$tick > 0 & d$tick <= 1200, ]

  fig <- ggplot(d, aes(tick, n, colour = species)) +
    geom_line(linewidth = 0.5) +
    theme_minimal() +
    labs(title = "Predator and prey without space: the wolves trail the sheep",
         subtitle = "mass action; period about 600 ticks, lag about 150",
         x = "tick", y = "population", colour = NULL)
  print(fig)
  ggsave(fig_file("56-predator-prey.png"), fig, width = 6, height = 4, dpi = 120)
}
