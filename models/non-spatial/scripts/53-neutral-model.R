...
if (sys.nframe() == 0L) {
  ...
  sweep <- do.call(rbind, lapply(
    list(c(500, 0.001), c(500, 0.005), c(500, 0.01), c(500, 0.05),
         c(2000, 0.0025)), function(par) {
      n <- par[1]; mu <- par[2]
      ...
      data.frame(n = n, mu = mu, observed = k, ewens = ewens_k(n, mu))
    }))

  # --- figure -------------------------------------------------------------
  # Parameter sweep: the standing variant count against Ewens's sampling
  # formula, which is the analytic result random copying is supposed to match.
  library(ggplot2)
  ...
  fig <- ggplot(sweep, aes(ewens, observed)) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed",
                colour = "grey50") +
    geom_point(size = 2) +
    theme_minimal() +
    labs(title = "Random copying lands on the Ewens sampling formula",
         subtitle = "five parameterisations, 3000 ticks; dashed line is equality",
         x = "Ewens expectation", y = "observed distinct variants")
  print(fig)
  ggsave(fig_file("53-neutral-model.png"), fig, width = 6, height = 4,
         dpi = 120)
}
