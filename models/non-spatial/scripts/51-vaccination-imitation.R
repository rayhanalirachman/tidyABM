...
if (sys.nframe() == 0L) {
  ...
  for (nt in c("random", "poisson", "scale_free")) {
    for (cc in c(0.05, 0.1, 0.2, 0.4, 0.8)) {
      ...
      sweep[[length(sweep) + 1]] <- data.frame(network = nt, cost = cc,
                                               coverage = o[1], attack = o[2])
    }
  }
  sweep <- do.call(rbind, sweep)

  # --- figure -------------------------------------------------------------
  # Parameter sweep: vaccine uptake against its relative cost, on the three
  # network types. Uptake falls with cost, and the network shape matters.
  library(ggplot2)
  ...
  fig <- ggplot(sweep, aes(cost, coverage, colour = network)) +
    geom_line(linewidth = 0.7) + geom_point() +
    ylim(0, 1) +
    theme_minimal() +
    labs(title = "Vaccination by imitation: uptake falls as the vaccine costs more",
         subtitle = "500 agents, mean degree 6, r = 0.4, g = 0.4, 100 seasons",
         x = "relative cost of vaccinating", y = "share vaccinated",
         colour = NULL)
  print(fig)
  ggsave(fig_file("51-vaccination-imitation.png"), fig, width = 6, height = 4,
         dpi = 120)
}
