...
run against several worlds, or one world run through several go blocks.
...
An experiment is the same third statement with two more arguments. `params`
overrides the model's globals, one run per value and every combination of
several; `reps` repeats each of those.
...
Sixty runs, stacked into one result with `.run`, `.rep` and a `step` column in
front of the ordinary ones. A single run is untouched: no `.run`, no `.rep`, the
same columns it always returned. One seed covers the whole thing, a seed per run
being derived from it, so an experiment reproduces at once rather than run by
run.

A parameter is a global and only a global. `n`, `abm_network()`'s `degree` and a
lattice's `dims` are fixed when `abm_setup()` builds the world, so sweeping one
of those still means one world per value. `reps` replicates the run, not the
population: a starting column drawn with `~runif(n)` was drawn before `abm_run()`
ever saw it, and what varies across replicates is what the run itself draws.

`measures` is how you get numbers out of sixty runs without keeping sixty
populations. Each one is a one-sided formula evaluated over the whole population
once per tick:
...
distinction: writing the same summary with `abm_global()` would put it where
every rule in the model can read it. A measure goes to `abm_measures()` and
nowhere else.
